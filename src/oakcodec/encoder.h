/***
  ...
  Oak Video Editor - Encoder Base
  Copyright (C) 2025 mikesolar

  Decoupled from node/ and render/ modules for oakcodec.so.

***/

#ifndef OAKCODEC_ENCODER_H
#define OAKCODEC_ENCODER_H
...
private:
    ...
    QString color_transform_output_;
...
public:
    ...
    // Subtitle writing is not supported in oakcodec.so standalone mode.
    // Subtitle blocks are a node-graph concept. Callers should handle
    // subtitle muxing through the node graph before encoding.
    virtual bool WriteSubtitle(const SubtitleParams &sub_params) = 0;
