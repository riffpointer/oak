...
AVFramePtr Decoder::RetrieveVideo(const RetrieveVideoParams &p)
{
	QMutexLocker locker(&mutex_);
	...
	if (cached_frame_ && cached_time_ == p.time &&
		cached_divider_ == p.divider) {
		return cached_frame_;
	}

	cached_frame_ = RetrieveVideoInternal(p);
	cached_time_ = p.time;
	cached_divider_ = p.divider;
...
QVector<DecoderPtr> Decoder::ReceiveListOfAllDecoders()
{
	...
	decoders.push_back(DecoderPtr(new OIIODecoder()));
	decoders.push_back(DecoderPtr(new FFmpegDecoder()));
...
AVFramePtr Decoder::RetrieveVideoInternal(const RetrieveVideoParams &p)
{
	Q_UNUSED(p)
	return nullptr;
}
