...
AVFramePtr FFmpegDecoder::ProcessFrameIntoBuffer(AVFramePtr f,
															   ...
															   const AVFramePtr original)
{
	...
	int dst_h = f->height / p.divider;
	if (dst_w < 1) dst_w = 1;
	if (dst_h < 1) dst_h = 1;

	// Re-create swscale context if parameters changed
	if (!sws_ctx_ ||
		sws_src_width_ != f->width ||
		sws_src_height_ != f->height ||
		sws_src_format_ != static_cast<AVPixelFormat>(f->format) ||
		sws_dst_width_ != dst_w ||
		sws_dst_height_ != dst_h ||
		sws_dst_format_ != dst_fmt ||
		sws_colrange_ != f->color_range ||
		sws_colspace_ != f->colorspace) {
		FreeScaler();
		...
		if (!sws_ctx_) {
			qCritical() << "Failed to create swscale context";
			return nullptr;
		}
		// Set color space / range if supported
		int *inv_table = nullptr;
		int *table = nullptr;
		int src_range, dst_range, brightness, contrast, saturation;
		if (sws_getColorspaceDetails(sws_ctx_, &inv_table, &src_range,
									 &table, &dst_range,
									 &brightness, &contrast, &saturation) >= 0) {
			src_range = (f->color_range == AVCOL_RANGE_JPEG) ? 1 : 0;
			sws_setColorspaceDetails(sws_ctx_, inv_table, src_range,
									table, dst_range,
	...
	AVFramePtr dst = CreateAVFramePtr();
	dst->width = dst_w;
	dst->height = dst_h;
	dst->format = dst_fmt;
	...
	// Scale / convert
	sws_scale(sws_ctx_, f->data, f->linesize, 0, f->height,
			  dst->data, dst->linesize);

	return dst;
}

AVFramePtr FFmpegDecoder::RetrieveVideoInternal(const RetrieveVideoParams &p)
{
	if (AVFramePtr f = RetrieveFrame(p.time, p.cancelled)) {
		...
		AVFramePtr frame_buf = ProcessFrameIntoBuffer(f, p, original);
