...
bool FFmpegEncoder::WriteSubtitle(const SubtitleParams &sub_params)
{
	bool ret = true;
	for (const Subtitle &sub : sub_params) {
		QByteArray utf8_sub = sub.text().toUtf8();
		...
		pkt->pts = Timecode::time_to_timestamp(
			sub.time().in(), subtitle_codec_ctx_->time_base, Timecode::kFloor);
		pkt->duration =
			av_rescale_q(qRound64(sub.time().length().toDouble() * 1000),
									 ...
									 const ExportCodec::Codec &codec)
{
	...
	if (type == AVMEDIA_TYPE_VIDEO) {
		...
		// Set custom options
		{
			...
			if (params().color_transform_output().contains(
