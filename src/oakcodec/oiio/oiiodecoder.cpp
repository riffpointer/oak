...
// Helper: convert olive::Frame to AVFramePtr (RGBA F32)
static AVFramePtr FrameToAVFrame(const olive::Frame &frame)
{
    AVFramePtr avf = CreateAVFramePtr();
    avf->width = frame.width();
    avf->height = frame.height();
    avf->format = AV_PIX_FMT_RGBAF32;
    if (av_frame_get_buffer(avf.get(), 0) < 0) {
        return nullptr;
    }
    int linesize = frame.linesize_bytes();
    int dst_linesize = avf->linesize[0];
    for (int y = 0; y < frame.height(); y++) {
        memcpy(avf->data[0] + y * dst_linesize,
               frame.const_data() + y * linesize,
               std::min(linesize, dst_linesize));
    }
    return avf;
}
...
AVFramePtr OIIODecoder::RetrieveVideoInternal(const RetrieveVideoParams &p)
{
	...
	if (vp.format() != PixelFormat::F32) {
		...
		if (f32_frame) {
			return FrameToAVFrame(*f32_frame);
		}
	}

	return FrameToAVFrame(buffer_);
}
