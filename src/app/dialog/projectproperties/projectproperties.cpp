	...
	, ocio_config_is_valid_(true)
{
...

	{
		...
		reference_space_->addItem(tr("Scene Linear"), QStringLiteral("scene_linear"));
...
void ProjectPropertiesDialog::OCIOFilenameUpdated()
{
	...
	if (ocio_filename_->text().isEmpty()) {
		c = ColorManager::GetDefaultConfigHandle();
	} else {
	...
	if (c) {
		...
		ocio_config_error_ = tr("Failed to load OCIO configuration");
	}
}
