	...
	, color_service_(nullptr)
{
	...
	if (RenderManager::instance()->backend() == RenderManager::kOpenGL) {
		...
		attached_renderer_ = new OpenGLRendererProxy(this);
...
void ManagedDisplayWidget::OnInit()
{
	if (RenderManager::instance()->backend() == RenderManager::kOpenGL) {
		...
		static_cast<OpenGLRendererProxy *>(attached_renderer_)->Init();
		static_cast<OpenGLRendererProxy *>(attached_renderer_)->PostInit();
	}
}
