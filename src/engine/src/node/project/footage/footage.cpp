	...
	if (!filename.isEmpty()) {
		...
		if (info.exists()) {
			...
			if (!QFileInfo::exists(meta_cache_file) ||
				...
				QVector<DecoderProxyPtr> decoder_list =
					DecoderProxy::ReceiveListOfAllDecoders();

				foreach (DecoderProxyPtr decoder, decoder_list) {
					footage_info = decoder->ProbeMedia(filename, decoder->id(), cancelled_);
