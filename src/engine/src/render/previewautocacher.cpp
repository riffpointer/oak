	...
	, ignore_cache_requests_(false)
{
	...
	// Catch when a conform is ready via C API callback
	auto rt = OakCodecRuntime::Instance();
	if (rt->Load() && rt->conform_set_ready_callback) {
		rt->conform_set_ready_callback(
			[](void* userdata) {
				auto* self = static_cast<PreviewAutoCacher*>(userdata);
				self->ConformFinished();
			},
			this);
	}
}
