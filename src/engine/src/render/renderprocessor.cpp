...
// STUB: openglrenderer removed, use oakgl.so C API at runtime
										 ...
										 const CodecStream &stream)
{
	...
	DecoderProxyPtr dec = nullptr;

	if (decoder.decoder && decoder.last_modified == file_last_modified) {
		dec = decoder.decoder;
	} else {
		// No decoder
		decoder.decoder = dec = DecoderProxy::CreateFromID(decoder_id);
										  ...
										  const rational &input_time)
{
	...
	if (decoder && render_ctx_) {
		DecoderProxy::RetrieveVideoParams p;
		...
		if (!IsCancelled()) {
			...
			if (tex_params.is_valid()) {
				AVFramePtr retrieved_frame = decoder->RetrieveVideo(p);
				TexturePtr unmanaged_texture;
				if (retrieved_frame) {
					unmanaged_texture = std::make_shared<Texture>(tex_params);
					unmanaged_texture->handleFrame(retrieved_frame);
				}
											 ...
											 const Node *node)
{
	...
	plugin::PluginRenderer *plugin_renderer = nullptr;
	{
		...
		if (!cached_plugin_renderer) {
			// PluginRenderer requires direct OpenGLRenderer context access.
			// This is temporarily disabled until PluginRenderer is moved out of engine.
